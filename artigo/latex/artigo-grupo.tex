...
\begin{abstract}
  ...
  from IBGE/SIDRA Table 1612 (RS municipalities, 2000--2025); the Oceanic
...
\begin{resumo}
  ...
  da Tabela 1612 do IBGE/SIDRA (municípios do RS, 2000--2025); o Oceanic
...
\begin{itemize}
  \item Consolidar uma base analítica integrada em nível
    ...
    na Tabela 1612 do IBGE/SIDRA (2000--2025);
...
unidade: a série de cada município cobre cerca de 26 safras (2000--2025),
...
A base final tem como chaves o código IBGE do município e a safra. Os dados
brutos do SIDRA foram obtidos via API pública do IBGE (tabela 1612,
variáveis de área plantada, área colhida, produção e rendimento médio,
categoria soja em grão) para os 497 municípios do RS, 2000--2025 (26
safras), e reorganizados para o formato município $\times$ ano; o ONI foi
agregado por janela de safra (outubro do ano anterior a março do ano da
safra), também disponível até 2025; e os dados diários do INMET (45
estações automáticas do RS, 2000--2025, obtidos via BDMEP em duas etapas)
foram associados a cada município pela estação mais próxima (distância de
Haversine) e agregados na mesma janela de safra (soma de precipitação,
média de temperatura), com combinações estação--safra de janela incompleta
(menos de 150 dos $\sim$182 dias esperados, por falha de equipamento ou
estação recém-instalada) tratadas como ausentes em vez de soma parcial. O
painel município--safra resultante foi implementado
em um notebook Python (disponível no GitHub do projeto) e a análise
exploratória cobriu: cobertura temporal dos dados (percentual de municípios
com rendimento informado por safra), distribuição do rendimento, série
temporal do rendimento médio estadual comparado ao ONI, e correlação entre
rendimento e as demais variáveis -- reproduzindo, com a base do RS, o tipo
de verificação já documentado na literatura (Pilau et al., 2022). Os
resultados dessa análise exploratória são detalhados na
Seção~\ref{sec:resultados-parciais}.
...
Seção~\ref{sec:dataset} foram efetivamente coletadas e documentadas, e o
painel município--safra foi construído e analisado em um notebook Python.
Os principais resultados dessa primeira análise exploratória são:

\begin{itemize}
  \item \textbf{Cobertura}: o percentual de municípios do RS com produção de
    soja registrada na safra cresce de 70,4\% em 2000 para 87,3\% em 2025,
    refletindo a expansão histórica da cultura no estado -- um ponto de
    atenção para a interpretação de médias estaduais nos anos iniciais da
    série.
  \item \textbf{Verificação de sanidade dos dados}: a série de rendimento
    médio estadual reproduz corretamente eventos climáticos conhecidos,
    com destaque para a quebra de safra de 2004/2005 (rendimento médio de
    apenas 662 kg/ha, o menor da série), coincidindo com a grande seca
    histórica registrada no Rio Grande do Sul naquele período.
  \item \textbf{Correlações preliminares} (nível município-safra, sem
    separar por município): rendimento $\times$ ONI médio da safra
    $\approx 0{,}22$; rendimento $\times$ precipitação acumulada na safra
    $\approx 0{,}38$. Ambas fracas a moderadas, o que é esperado em uma
    análise agregada e simples, e reforça a escolha de modelos capazes de
    capturar interações não lineares (Random Forest, Gradient Boosting) na
    próxima etapa, em vez de depender de uma relação linear direta.
  \item \textbf{Limitações já identificadas} nesta primeira versão da base:
    (i) a janela do ONI usada (OND+NDJ do ano anterior e DJF+JFM do ano da
    safra) é uma primeira aproximação a validar; (ii) municípios distantes
    das 45 estações do INMET têm o clima estimado por uma estação
    potencialmente distante; (iii) períodos em que a estação estava em
    pane geram dados ausentes, e janelas de safra com menos de 150 dos
    $\sim$182 dias esperados de dado são tratadas como ausentes (não como
    soma parcial); (iv) a adubação fosfatada (Embrapa/CQFS-RS/SC) ainda não
    foi integrada ao painel, por ser uma tabela de referência por classe de
    solo e não uma série real por município -- sua incorporação exigirá
    uma hipótese explícita de mapeamento região--classe de solo, a ser
    definida na próxima etapa.
\end{itemize}

O código completo desta etapa (coleta, integração e análise exploratória)
está disponível no notebook \texttt{notebooks/analise\_exploratoria.ipynb}
do repositório do projeto (Seção~\ref{sec:github}).

\subsection{Endereço GitHub}
\label{sec:github}

\url{https://github.com/isah5/Projeto_Safra_de_Soja}
